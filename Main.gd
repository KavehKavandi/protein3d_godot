...
func _http_request_completed(result, response_code, headers, body):
	if response_code == 200:
		atoms.append({"name": "Start", "atom_name": "", "position": Vector3(0, 0, 0), "isTer": true})
		...
		for line in lines:
			if line.begins_with("ATOM") or line.begins_with("HETATM"):
				...
				atoms.append({"name": "Atom" + str(atom_number), "atom_name": atom_name, "position": atom_position, "isTer": false})
				
				if last_atom_position != null and !line.begins_with("HETATM"):
					var atom_a_position = last_atom_position
					var atom_b_position = atom_position
				...
				last_atom_position = atom_position
				
			elif line.begins_with("TER"):
				last_atom_position = null
				var atom_number = line.substr(7, 4).strip_edges().to_int()
				atoms.append({"name": "Ter" + str(atom_number), "atom_name": "", "position": Vector3(0, 0, 0), "isTer": true})
		...
		atoms.append({"name": "End", "atom_name": "", "position": Vector3(0, 0, 0), "isTer": true})
	...
	$Atoms.multimesh = MultiMesh.new()
	$Atoms.multimesh.transform_format = MultiMesh.TRANSFORM_3D
	$Atoms.multimesh.use_colors = true
	$Atoms.multimesh.use_custom_data = true
	$Atoms.multimesh.mesh = SphereMesh.new()
	$Atoms.multimesh.mesh.material = StandardMaterial3D.new()
	$Atoms.multimesh.mesh.material.vertex_color_use_as_albedo  = true
	$Atoms.multimesh.mesh.material.albedo_color = Color(1, 1, 1)
	$Atoms.multimesh.instance_count = atoms.size()
	$Atoms.multimesh.visible_instance_count = atoms.size()
	
	for i in range($Atoms.multimesh.visible_instance_count):
		if !atoms[i].isTer:
			var position = Transform3D()
			position = position.translated(Vector3(atoms[i].position.x, atoms[i].position.y, atoms[i].position.z))
			$Atoms.multimesh.set_instance_transform(i, position)
			var atom_color = atom_default_color
			if atoms_colors.has(atoms[i].atom_name.capitalize()):
				atom_color = atoms_colors[atoms[i].atom_name.capitalize()]
			$Atoms.multimesh.set_instance_color(i, atom_color)
	
	$UserInterface/BackButton.disabled = false
	

func midpoint(point_a, point_b):
